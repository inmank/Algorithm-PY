...
def countComparisonUsingQuickSort(inList, start, end):
    ...
    if (len(inList[start:pivotIndex]) > 1):
        countComparisonUsingQuickSort(inList, start, pivotIndex)
    
    if (len(inList[pivotIndex+1:end]) > 1):
        countComparisonUsingQuickSort(inList, pivotIndex+1, len(inList))
...
'''
print countComparisonUsingQuickSort(valuesIn, 0, len(valuesIn))
